...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS public_payments (id SERIAL PRIMARY KEY, payment_reference TEXT NOT NULL UNIQUE, file_number TEXT NOT NULL, name TEXT NOT NULL, email TEXT NOT NULL, amount NUMERIC(12,2) NOT NULL, currency TEXT NOT NULL DEFAULT 'usd', status TEXT NOT NULL DEFAULT 'pending', stripe_session_id TEXT, paid_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());`);
    await client.query(`CREATE TABLE IF NOT EXISTS chat_conversations (id SERIAL PRIMARY KEY, name TEXT NOT NULL, email TEXT NOT NULL, file_number TEXT, status TEXT NOT NULL DEFAULT 'open', last_message_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());`);
    await client.query(`CREATE TABLE IF NOT EXISTS chat_messages (id SERIAL PRIMARY KEY, conversation_id INTEGER NOT NULL REFERENCES chat_conversations(id) ON DELETE CASCADE, sender TEXT NOT NULL, message TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW());`);
