...
export async function generateWorkPermitDecisionPdf(
  data: WorkPermitDecisionData
): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.font("Bold").fontSize(9.5)
      .text("REPUBLICA POPULARA", natCol, ab1Y)
    ...
    const ab2Y = 418;
    doc.font("Bold").fontSize(9.5).text("cet.", ML, ab2Y);
    doc.font("Bold").fontSize(9.5)
    ...
    const rowY = 458;
    const dobVal = fmtDOB(data.dateOfBirth);
    ...
    doc.font("Bold").fontSize(9.5).text(",pasaport national seria", ML + 162, rowY);
    doc.font("Bold").fontSize(9.5).text("A", ML + 288, rowY);
    doc.font("Bold").fontSize(9.5).text("nr.", ML + 302, rowY);
    ...
    doc.font("Regular").fontSize(7.5).fillColor("#111")
      .text(
        "  In conformitate cu prevederile art. 164, alin. (1) al Codului Administrativ al Republicii Moldova nr. 116 din " +
        "19.07.2018 sunteti in drept sa depuneti cererea prealabila in termen de 30 de zile la comunicare, pentru a contesta " +
        "decizia Inspectoratului General pentru Migratie. Cererea prealabila se depune la secretariatul Inspectoratului " +
        "General pentru Migratie, situat pe adresa: mun. Chisinau, str. Stefan cel Mare 124.",
        ML, footerRuleY + 6, { width: CW, align: "justify", lineGap: 1.5 }
      );
    const p2Y = doc.y + 5;
    doc.font("Regular").fontSize(7.5).text(
      "  Informatia din acest document contine date cu caracter personal si necesita a fi prelucrata si protejata in " +
      "conformitate cu Legea nr. 133 din 08.07.2011 privind protectia datelor cu caracter personal.",
      ML, p2Y, { width: CW, align: "justify", lineGap: 1.5 }
    );
    doc.end();
  });
}
