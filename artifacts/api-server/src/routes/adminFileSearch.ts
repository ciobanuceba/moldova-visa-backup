...
router.get("/admin/file-search/:referenceNumber", async (req, res): Promise<void> => {
  ...
  try {
    const { rows } = await client.query(
      `SELECT id, reference_number, first_name, last_name, email, phone, permit_type,
              employer_name, employer_country, job_title, status, payment_status,
              admin_notes, start_date, contract_duration, created_at
       FROM work_permits WHERE reference_number = $1 LIMIT 1`,
      [referenceNumber]
    );
...
router.patch("/admin/file-search/:id", async (req, res): Promise<void> => {
  ...
  const allowed: Record<string, string> = {
    status: "status", employerName: "employer_name", employerCountry: "employer_country",
    jobTitle: "job_title", startDate: "start_date", contractDuration: "contract_duration", adminNotes: "admin_notes",
  };
  ...
  try {
    const { rows } = await client.query(
      `UPDATE work_permits SET ${setClauses.join(", ")} WHERE id = $${index} RETURNING
       id, reference_number, first_name, last_name, email, phone, permit_type,
       employer_name, employer_country, job_title, status, payment_status,
       admin_notes, start_date, contract_duration, created_at`, values);
    if (!rows.length) { res.status(404).json({ error: "Application not found" }); return; }
