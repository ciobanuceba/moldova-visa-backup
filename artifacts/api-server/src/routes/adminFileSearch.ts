...
router.get("/admin/file-search/:referenceNumber", async (req, res): Promise<void> => {
  ...
  try {
    const { rows } = await client.query(`SELECT ${selectFields} FROM work_permits WHERE reference_number = $1 LIMIT 1`, [referenceNumber]);
...
router.patch("/admin/file-search/:id", async (req, res): Promise<void> => {
  ...
  for (const [key,column] of Object.entries(map)) if (b[key] !== undefined) {
    ...
    if (["hasPassport","hasJobOffer","hasMedicalCert","hasCriminalRecord","hasPhotos","hasEducationCert"].includes(key)) values.push(Boolean(b[key]));
    else if (["passportCopyData","photoData","medicalCertData","criminalRecordData"].includes(key)) values.push(b[key] === null || b[key] === "" ? null : dataUrl(b[key]));
    else values.push(b[key] === null ? null : text(b[key]));
  }
  if (!clauses.length) { res.status(400).json({ error: "No fields to update" }); return; }
  values.push(id);
  const client=await pool.connect();
  try { const {rows}=await client.query(`UPDATE work_permits SET ${clauses.join(", ")} WHERE id = $${i} RETURNING ${selectFields}`,values); if(!rows.length){res.status(404).json({error:"Application not found"});return;} res.json({success:true,application:rows[0]}); }
