...
router.post("/payments/work-permit/checkout", requireApplicant, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: { currency: "eur", unit_amount: WORK_PERMIT_FEE_EUR, product_data: { name: "Work Permit Fee" } },
        quantity: 1,
      }],
      mode: "payment",
    ...
    // Stripe Session ID আপডেট করা যাতে পরে ভেরিফাই করা যায়
...
router.get("/payments/work-permit/verify", async (req, res): Promise<void> => {
  ...
  if (session.payment_status === "paid") {
    ...
    try {
      await client.query(
        `UPDATE work_permits 
         SET payment_status = 'pending_review', status = 'payment_confirmed' 
