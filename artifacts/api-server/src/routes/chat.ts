...
router.post("/chat/conversations", async (req, res): Promise<void> => {
  ...
  try {
    ...
    res.status(201).json({ conversation, messages: [m.rows[0]] });
...
router.post("/chat/conversations/:id/messages", async (req, res): Promise<void> => {
  const id = Number(req.params.id); const { message, sender = "user" } = req.body ?? {};
  if (!Number.isInteger(id) || !message || !["user", "admin"].includes(sender)) { res.status(400).json({ error: "Invalid message" }); return; }
  const client = await pool.connect();
  try {
    ...
    const m = await client.query(`INSERT INTO chat_messages (conversation_id,sender,message) VALUES ($1,$2,$3) RETURNING *`, [id, sender, String(message).trim()]);
    await client.query(`UPDATE chat_conversations SET last_message_at=NOW() WHERE id=$1`, [id]);
    const convo = c.rows[0];
    if (sender === "user") sendEmail({ to: adminEmail(), subject: `New Live Chat Message — ${convo.name}`, html: `<h2>New Live Chat Message</h2><p><b>${convo.name}</b> (${convo.email})</p><p>${String(message).trim()}</p><p>Open the Admin Dashboard to reply.</p>` }).catch(() => {});
