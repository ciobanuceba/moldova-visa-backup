...
function getApplicationReference(app: {
  firstName: string;
  lastName: string;
  jobTitle: string;
  location: string;
  ...
  const digest = createHash("sha256")
    .update([applicantName, app.jobTitle, app.location, app.salary, app.startDate || ""].join("|"))
    .digest("hex")
    .slice(0, 10)
    .toUpperCase();
...
router.post("/applications", async (req, res): Promise<void> => {
  ...
  const location = sanitizeString(body.location) ?? "Europe";
  const salary = sanitizeString(body.salary) ?? "As discussed";
  const referenceNumber = getApplicationReference({
    firstName,
    lastName,
    jobTitle,
    location,
  ...
  sendEmail({
    ...
    html: `${applicationReceivedEmail(firstName, jobTitle)}<p style="font-family:Arial,sans-serif"><strong>Reference number:</strong> ${referenceNumber}</p>`,
  }).then(() => {
    logger.info({ to: email, referenceNumber }, "User confirmation email sent successfully");
  }).catch((err) => {
  ...
  if (adminEmail) {
    const adminHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
        <div style="background: #1a2744; padding: 20px; text-align: center; color: white;"><h2 style="margin: 0;">New Visa Application Submitted</h2></div>
        <div style="padding: 24px; background: #ffffff; color: #374151; line-height: 1.6;">
          <p>Hi Admin,</p><p>A new job application has been successfully submitted.</p>
          <p><strong>Reference:</strong> ${referenceNumber}</p>
          <table style="width:100%;border-collapse:collapse;margin-top:16px;">
            <tr style="background:#f9fafb"><td style="padding:8px;font-weight:bold">Full Name:</td><td style="padding:8px">${firstName} ${lastName}</td></tr>
          ...
          ${resumeUrl ? `<div style="margin-top:20px;padding:12px;background:#eff6ff;border-radius:6px;text-align:center"><p style="margin:0 0 8px;font-weight:bold">Submitted Resume / PDF:</p><a href="${resumeUrl}" target="_blank" style="background:#2563eb;color:white;text-decoration:none;padding:8px 16px;border-radius:4px;display:inline-block">View Document / PDF</a></div>` : ""}
    ...
    sendEmail({
      to: adminEmail,
      subject: `🚨 New Application: ${firstName} ${lastName} [${referenceNumber}]`,
      html: adminHtml,
