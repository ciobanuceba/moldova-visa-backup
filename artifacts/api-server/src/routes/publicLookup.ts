...
router.get("/public/applications/:referenceNumber", async (req,res):Promise<void> => {
  ...
  try {
    ...
    res.json({found:true,application:{referenceNumber:permit.referenceNumber,applicantName:`${permit.firstName} ${permit.lastName.slice(0,1)}.`,jobTitle:permit.jobTitle,employerName:permit.employerName,employerCountry:permit.employerCountry,startDate:permit.startDate,contractDuration:permit.contractDuration,employerLogo:permit.employerLogoData,status:publicStatus,createdAt:permit.createdAt}});
