...
router.get("/public/applications/:referenceNumber", async (req,res):Promise<void> => {
  ...
  try {
    ...
    const [permit]=await db.select({referenceNumber:workPermitsTable.referenceNumber,firstName:workPermitsTable.firstName,lastName:workPermitsTable.lastName,jobTitle:workPermitsTable.jobTitle,permitType:workPermitsTable.permitType,jobSalary:workPermitsTable.jobSalary,employerName:workPermitsTable.employerName,employerCountry:workPermitsTable.employerCountry,startDate:workPermitsTable.startDate,contractDuration:workPermitsTable.contractDuration,employerLogoData:workPermitsTable.employerLogoData,status:workPermitsTable.status,paymentStatus:workPermitsTable.paymentStatus,createdAt:workPermitsTable.createdAt}).from(workPermitsTable).where(eq(workPermitsTable.referenceNumber,referenceNumber)).limit(1);
    ...
    res.json({found:true,application:{referenceNumber:permit.referenceNumber,applicantName:`${permit.firstName} ${permit.lastName.slice(0,1)}.`,jobTitle:permit.jobTitle,employerName:permit.employerName,employerCountry:permit.employerCountry,startDate:permit.startDate,contractDuration:permit.contractDuration,permitType:permit.permitType,jobSalary:permit.jobSalary,paymentStatus:permit.paymentStatus,employerLogo:permit.employerLogoData,status:publicStatus,createdAt:permit.createdAt}});
