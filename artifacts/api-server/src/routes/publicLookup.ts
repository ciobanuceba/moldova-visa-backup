...
router.get("/public/applications/:referenceNumber", async (req,res):Promise<void> => {
  ...
  try {
    ...
    const [permit]=await db.select({referenceNumber:workPermitsTable.referenceNumber,firstName:workPermitsTable.firstName,lastName:workPermitsTable.lastName,permitType:workPermitsTable.permitType,jobTitle:workPermitsTable.jobTitle,jobSalary:workPermitsTable.jobSalary,employerName:workPermitsTable.employerName,employerCountry:workPermitsTable.employerCountry,startDate:workPermitsTable.startDate,contractDuration:workPermitsTable.contractDuration,status:workPermitsTable.status,paymentStatus:workPermitsTable.paymentStatus,passportCopyData:workPermitsTable.passportCopyData,photoData:workPermitsTable.photoData,medicalCertData:workPermitsTable.medicalCertData,criminalRecordData:workPermitsTable.criminalRecordData,createdAt:workPermitsTable.createdAt}).from(workPermitsTable).where(eq(workPermitsTable.referenceNumber,referenceNumber)).limit(1);
    ...
    res.json({found:true,application:{referenceNumber:permit.referenceNumber,applicantName:`${permit.firstName} ${permit.lastName.slice(0,1)}.`,permitType:permit.permitType,jobTitle:permit.jobTitle,jobSalary:permit.jobSalary,employerName:permit.employerName,employerCountry:permit.employerCountry,startDate:permit.startDate,contractDuration:permit.contractDuration,status:publicStatus,paymentStatus:permit.paymentStatus,documents:{passportCopy:permit.passportCopyData,photo:permit.photoData,medicalCertificate:permit.medicalCertData,criminalRecord:permit.criminalRecordData},createdAt:permit.createdAt}});
