...
export default function ApplicationLookup() {
  ...
  return <div className="min-h-[70vh] bg-slate-50 px-4 py-12"><div className="mx-auto max-w-2xl"><div className="rounded-2xl border bg-white p-6 shadow-sm sm:p-8">
    <div className="mb-8 text-center"><div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-xl bg-[#1a2744] text-2xl text-white">MVA</div><h1 className="text-2xl font-bold text-[#1a2744]">Check Application</h1><p className="mt-2 text-sm text-slate-500">Enter your Moldova Visa Assist application reference number.</p></div>
    ...
    {result && <div className="mt-6 rounded-xl border bg-slate-50 p-5"><div className="mb-5 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Reference</p><p className="font-mono font-bold text-[#1a2744]">{result.referenceNumber}</p></div><span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-semibold">{result.status}</span></div><div className="grid gap-4 sm:grid-cols-2"><Info label="Applicant" value={result.applicantName} /><Info label="Application" value={result.jobTitle} /><Info label="Destination" value={result.location || result.employerCountry || "—"} />{result.startDate && <Info label="Travel Date" value={result.startDate} />}</div><p className="mt-5 border-t pt-4 text-xs text-slate-500">Only limited public application information is displayed. Passport, contact, payment and internal administrative information are not displayed.</p></div>}
  </div></div></div>;
}
