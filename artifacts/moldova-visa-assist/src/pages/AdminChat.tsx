...
export default function AdminChat() {
  ...
  async function loadConversations() {
    if (!user) return;
    const res = await fetch("/api/admin/chat/conversations", { headers: authHeaders(user.token) });
  ...
  useEffect(() => { const t = window.setInterval(() => { loadConversations(); if (selected) loadMessages(selected); }, 3000); return () => window.clearInterval(t); }, [selected, user]);
