...
export default function FileNumberSearch() {
  ...
  return <div className="min-h-[70vh] bg-slate-50 px-4 py-12"><div className="mx-auto max-w-2xl"><div className="rounded-2xl border bg-white p-6 shadow-sm sm:p-8">
    <div className="mb-7 text-center"><h1 className="text-2xl font-bold text-[#1a2744]">File Number Search</h1><p className="mt-2 text-sm text-slate-500">Enter your File Number to view the latest application status and employer information.</p></div>
    ...
    {result && <div className="mt-6 rounded-xl border bg-slate-50 p-5"><div className="mb-5 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">File Number</p><p className="font-mono font-bold text-[#1a2744]">{result.referenceNumber}</p></div><span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-semibold">{result.status}</span></div><div className="grid gap-4 sm:grid-cols-2"><Info label="Applicant" value={result.applicantName}/><Info label="Job / Position" value={result.jobTitle}/><Info label="Employer" value={result.employerName}/><Info label="Employer Country" value={result.employerCountry}/><Info label="Start Date" value={result.startDate}/><Info label="Contract Duration" value={result.contractDuration}/></div></div>}
  </div></div></div>;
}
