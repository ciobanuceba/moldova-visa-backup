...
export default function Home() {
  ...
  useEffect(() => { const timer = window.setInterval(() => setHeroIndex((i) => (i + 1) % heroImages.length), 5000); return () => window.clearInterval(timer); }, []);
  ...
  return <div className="flex flex-col min-h-screen">
    <section className="relative bg-primary pt-16 pb-24 overflow-hidden"><div className="absolute inset-0 z-0 bg-gradient-to-br from-primary via-primary to-primary/90" /><div className="container mx-auto px-4 md:px-8 relative z-10"><div className="max-w-5xl mx-auto"><div className="relative w-full aspect-[16/9] md:aspect-[2/1] overflow-hidden rounded-3xl border border-white/20 shadow-2xl bg-black/20">{heroImages.map((src, index) => <img key={src} src={src} alt={`Moldova vineyard worker ${index + 1}`} className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${index === heroIndex ? "opacity-100" : "opacity-0"}`} />)}<div className="absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-transparent pointer-events-none" /><div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-2 z-10" aria-label="Hero slideshow controls">{heroImages.map((_, index) => <button key={index} type="button" onClick={() => setHeroIndex(index)} aria-label={`Show slide ${index + 1}`} className={`h-2.5 rounded-full transition-all ${index === heroIndex ? "w-9 bg-secondary" : "w-2.5 bg-white/70"}`} />)}</div></div><div className="flex flex-col sm:flex-row justify-center gap-4 mt-8"><Button size="lg" variant="secondary" className="font-semibold px-8" asChild><Link href="/jobs">{t.home.browseJobs} <ArrowRight className="ml-2 h-5 w-5" /></Link></Button><Button size="lg" variant="outline" className="font-semibold bg-white/10 text-white border-white/20 hover:bg-white/20" asChild><Link href="/services">{t.home.learnVisa}</Link></Button></div></div></div></section>
