...
export default function AdminManual() {
  ...
  async function createOffer(e: React.FormEvent) {
    ...
    try {
      ...
      const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `Manual_Job_Offer_${Date.now()}.pdf`; a.click(); URL.revokeObjectURL(url);
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <form onSubmit={createOffer} className="bg-card border rounded-xl p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <Field label="Applicant name *" value={offer.applicantName} onChange={setO("applicantName")} />
            <Field label="Email" value={offer.email} onChange={setO("email")} type="email" />
            <Field label="Phone" value={offer.phone} onChange={setO("phone")} />
            <Field label="Nationality" value={offer.nationality} onChange={setO("nationality")} />
            <Field label="Date of birth" value={offer.dateOfBirth} onChange={setO("dateOfBirth")} />
            <Field label="Passport number" value={offer.passportNumber} onChange={setO("passportNumber")} />
            <Field label="Position *" value={offer.jobTitle} onChange={setO("jobTitle")} />
            <Field label="Location *" value={offer.location} onChange={setO("location")} />
            <Field label="Salary *" value={offer.salary} onChange={setO("salary")} />
            <Field label="Start date" value={offer.startDate} onChange={setO("startDate")} />
            <Field label="Experience" value={offer.yearsExperience} onChange={setO("yearsExperience")} />
            <Field label="Languages" value={offer.languages} onChange={setO("languages")} />
            <Field label="Skills" value={offer.skills} onChange={setO("skills")} />
            <Field label="Resume URL" value={offer.resumeUrl} onChange={setO("resumeUrl")} />
          </div>
        ...
        <form onSubmit={createPermit} className="bg-card border rounded-xl p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <Field label="First name *" value={permit.firstName} onChange={setP("firstName")} />
            <Field label="Last name *" value={permit.lastName} onChange={setP("lastName")} />
            <Field label="Email *" value={permit.email} onChange={setP("email")} type="email" />
            <Field label="Phone *" value={permit.phone} onChange={setP("phone")} />
            <Field label="Nationality *" value={permit.nationality} onChange={setP("nationality")} />
            <Field label="Date of birth *" value={permit.dateOfBirth} onChange={setP("dateOfBirth")} />
            <Field label="Passport number *" value={permit.passportNumber} onChange={setP("passportNumber")} />
            <Field label="Passport expiry *" value={permit.passportExpiry} onChange={setP("passportExpiry")} />
            <Field label="Current address *" value={permit.currentAddress} onChange={setP("currentAddress")} />
            <Field label="Permit type *" value={permit.permitType} onChange={setP("permitType")} />
            <Field label="Employer *" value={permit.employerName} onChange={setP("employerName")} />
            <Field label="Employer country *" value={permit.employerCountry} onChange={setP("employerCountry")} />
            <Field label="Job title *" value={permit.jobTitle} onChange={setP("jobTitle")} />
            <Field label="Salary *" value={permit.jobSalary} onChange={setP("jobSalary")} />
            <Field label="Start date *" value={permit.startDate} onChange={setP("startDate")} />
            <Field label="Contract duration *" value={permit.contractDuration} onChange={setP("contractDuration")} />
          </div>
          <div className="grid sm:grid-cols-2 gap-2 text-sm">
            {([['hasPassport','Passport'],['hasJobOffer','Job Offer'],['hasMedicalCert','Medical certificate'],['hasCriminalRecord','Criminal record'],['hasPhotos','Photos'],['hasEducationCert','Education certificate']] as const).map(([key,label]) => <label key={key} className="flex items-center gap-2"><input type="checkbox" checked={permit[key]} onChange={e => setPermit(p => ({ ...p, [key]: e.target.checked }))} />{label}</label>)}
          </div>
