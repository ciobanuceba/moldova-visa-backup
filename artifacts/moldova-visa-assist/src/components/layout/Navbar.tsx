...
import { Menu, X, Globe, Briefcase, FileText, Info, Mail, HelpCircle, Shield, PenLine, LayoutDashboard, LogIn } from "lucide-react";
...
export function Navbar() {
  ...
  const primaryNav = navItems.filter(i =>
    ["/", "/jobs", "/letter-builder", "/work-permit", "/about", "/contact"].includes(i.href)
  );
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 md:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          {/* Desktop controls */}
          <div className="hidden lg:flex items-center space-x-2 pl-4 border-l border-border shrink-0">
            <LanguageToggle />
            <ThemeToggle />
            {isApplicant ? (
              <Button asChild variant="secondary" className="font-medium rounded-full px-5 ml-1">
                <Link href="/dashboard">
              ...
              <>
                <Button asChild variant="ghost" size="sm" className="font-medium ml-1">
                  <Link href="/login"><LogIn className="w-4 h-4 mr-1" /> Sign In</Link>
                </Button>
                <Button asChild variant="secondary" className="font-medium rounded-full px-5">
                  <Link href="/jobs">{t.nav.findOpportunities}</Link>
                </Button>
              </>
            )}
          </div>
        ...
        <div className="lg:hidden border-b bg-background">
          <div className="space-y-1 px-4 pb-4 pt-2">
            ...
            <div className="pt-4 border-t border-border mt-2 space-y-2">
              {isApplicant ? (
                <Button asChild variant="secondary" className="w-full font-medium rounded-full">
                ...
                <>
                  <Button asChild variant="outline" className="w-full font-medium rounded-full">
