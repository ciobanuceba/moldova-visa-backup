...
export function LiveChat() {
  ...
  return <>
    ...
    {open && <div className="fixed bottom-5 right-5 z-[70] w-[calc(100vw-2rem)] max-w-sm h-[520px] bg-background border rounded-2xl shadow-2xl overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-muted/20">{messages.length === 0 ? <p className="text-center text-sm text-muted-foreground py-10">Our support team is here to help. Send us a message anytime.</p> : messages.map(m => <div key={m.id} className={`max-w-[82%] rounded-xl px-3 py-2 text-sm ${m.sender === "user" ? "ml-auto bg-primary text-primary-foreground" : "mr-auto bg-card border"}`}>{m.message}</div>)}</div>
